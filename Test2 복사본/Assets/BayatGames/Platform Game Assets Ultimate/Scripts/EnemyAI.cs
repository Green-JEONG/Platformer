...
namespace BayatGames
{
    public class EnemyAI : MonoBehaviour
    {
        ...
        [SerializeField]
        protected float attackDamage = 10f; // 공격 시 플레이어에게 가할 피해량
        ...
        {
            return Mathf.Abs(playerPoition - enemyPostion) < minRange;
        }

        public bool checkFollowRadius(Vector2 playerPosition, Vector2 enemyPosition)
        {
            return Vector2.Distance(playerPosition, enemyPosition) < followRange;
        }
        ...
        public void FollowCharacter()
        {
            if (checkFollowRadius(target.position, this.transform.position))
            {
                ...
                else
                {
                    ...
                    float speed = runWhenTargetInRange ? runSpeed : walkSpeed;
                    Move(new Vector2(Mathf.Sign(target.position.x - transform.position.x) * speed, 0));
                    animator.SetFloat("SpeedX", Mathf.Abs(rigidbody2d.velocity.x));
                }
